...
mock.module("./search", () => ({
  findRoute: (
    ...
    calls += 1;
    return { steps: [{ edge: chosenPath(tree, ferry, allow), forward: true }] };
  },
}));
...
test("the cache never returns a route that differs from a fresh search", () => {
  ...
  for (const [tree, ferry, allow] of sequence) {
    const cached = cache.route(GRAPH, START, DEST, tree, ferry, allow);
...
test("bracketing the active slider skips the search on a settled interval", () => {
  ...
  cache.route(GRAPH, START, DEST, 0.6, 0.1, true);
  cache.route(GRAPH, START, DEST, 1, 0.1, true);
  cache.route(GRAPH, START, DEST, 0.8, 0.1, true);
  expect(calls).toBe(2);
});
...
test("switching sliders drops the old range and rebrackets the new one", () => {
  calls = 0;
  const cache = new RouteCache();
  cache.route(GRAPH, START, DEST, 0.6, 0.1, true); // tree axis
  cache.route(GRAPH, START, DEST, 1, 0.1, true); // tree axis, two samples
  cache.route(GRAPH, START, DEST, 1, 0.8, true); // switch to ferry: seed + compute
  cache.route(GRAPH, START, DEST, 1, 0.8, true); // exact repeat: no search
  expect(calls).toBe(3);
  // Back to a tree weight already sampled on the dropped tree range — the range is gone, so it runs.
  cache.route(GRAPH, START, DEST, 0.6, 0.8, true);
  expect(calls).toBe(4);
});
