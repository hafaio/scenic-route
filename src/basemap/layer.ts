...
//
// This is still a plain Leaflet GridLayer painting canvas tiles, which matters for one specific
// reason: the dark-mode filter in app/globals.css inverts `.leaflet-tile-pane`, and it goes on
// working over these tiles exactly as it did over the raster ones. A native dark style is a second
// colour dictionary, and a separate piece of work.
...
export function basemapLayer(): L.Layer {
  return leafletLayer({
    url: BASEMAP_URL,
    ...
    maxZoom: 20,
    keepBuffer: 4,
    paintRules: basemapPaintRules(VOYAGER),
    labelRules: basemapLabelRules(VOYAGER),
    backgroundColor: VOYAGER.background as string,
